...
function get_jump_bound_fwrappers(u, p, t, jumps)
    BoundWrapper = FunctionWrappers.FunctionWrapper{typeof(t),
        Tuple{typeof(u), typeof(u), typeof(p), typeof(t)}}

    if (jumps !== nothing) && !isempty(jumps)
        lrates = [BoundWrapper(make_lower_bound_fn(c)) for c in jumps]
        urates = [BoundWrapper(make_upper_bound_fn(c)) for c in jumps]
    else
        lrates = Vector{BoundWrapper}()
        urates = Vector{BoundWrapper}()
    end

    lrates, urates
end

make_lower_bound_fn(c::ConstantRateJump) =
    (ulow, uhigh, p, t) -> lower_rate_bound(c, ulow, uhigh, p, t)

make_upper_bound_fn(c::ConstantRateJump) =
